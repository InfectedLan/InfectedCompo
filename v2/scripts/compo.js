	...
	$.getJSON('../api/json/getcompostatus.php', function(data){
		if(data.result == true) {
			...
			for(var i = 0; i < data.data.clans.length; i++) {
				$("#teamData").append('<h1 class="teamEntry" id="teamHeaderId' + data.data.clans[i].id + '">' + data.data.clans[i].tag + " - " + data.data.clans[i].compo.tag + '</h1>');
			...
			if(data.data.invites.length != 0) {
				...
				for(var i = 0; i < data.data.invites.length; i++) {
					$("#teamData").append('<div><h1>' + data.data.invites[i].clanTag + '</h1><h3>' + data.data.invites[i].compo.tag + '</h3><i class="teamEntry" id="inviteAccept' + data.data.invites[i].id + '">Godta</i> - <i class="teamEntry" id="inviteDecline' + data.data.invites[i].id + '">Avslå</i></div>');
